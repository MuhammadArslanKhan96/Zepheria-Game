...
export default class Creator {
    ...
    public async getPlayers() {
        let player_infocollection = this.database.collection('player_information');
        let players = await player_infocollection.find().toArray();
        return players
    }

    /**
     * Saves the complete information about the player.
     * See `PlayerInformations` for more information.
     */

    private savePlayer(player: Player): void {
        let collection = this.database.collection('player_information');

        this.getPlayerWithHash(player, (playerInfo: PlayerInfo) => {
            this.updateCollection(collection, player.username, { ...playerInfo, equipment: player.equipment.serialize().equipments, inventory: player.inventory.serialize(), bank: player.bank.serialize(), quests: player.quests.serialize().quests, achievements: player.achievements.serialize().achievements, skills: player.skills.serialize(), statistics: player.statistics.serialize(), abilities: player.abilities.serialize().abilities });
        });
    }
